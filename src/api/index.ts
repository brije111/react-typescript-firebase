...
export const writeChatData = (chat: Chat): void => {
    const node = getConcatenatedNode(chat.senderId, chat.receiverId);
    app.firestore().collection('chat').doc(node).collection('/chat')
    .add(chat).then(() => {
...
export const listenForChatChange = (senderId: string, receiverId:string, dataResult: ChatDataResult,
...
const getConcatenatedNode=(senderId:string, receiverId:string):string=>{
